// GigRadar Discovery Agent: finds jobs, decides which are safe and realistically hireable from Nigeria,
...
const {
    ...
    useLlm = true,
    maxLlmCalls = 10,
    seenJobsStoreName = 'gigradar-seen-jobs',
    resetSeenJobs = false,
} = input;
...
await Actor.setValue('RUN_PLAN', { targetRole, location, remotePreference, ...plan });
...
async function processListing(raw) {
    ...
    const tag = { available: 'AVAILABLE', scam_flagged: 'SCAM FLAG', unlikely_hireable: 'UNLIKELY HIREABLE' }[decision];
    log.info(
